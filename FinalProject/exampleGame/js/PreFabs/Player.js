//This Prefab is for the main character for the game
//Almost including everything include the collision and overlap
...
Players.prototype.update = function(){
	...
	if (cursors.up.isDown){
		if (!this.keyQueue.includes("Up")){
			this.keyQueue.unshift("Up");
		}
	}
	else {
		for( var i = 0; i < this.keyQueue.length; i++){ 
			if ( this.keyQueue[i] === "Up") {
			  this.keyQueue.splice(i, 1); 
			}
		 }
	}
	if (cursors.down.isDown){
		if (!this.keyQueue.includes("Down")){
			this.keyQueue.unshift("Down");
		}
	}
	else {
		for( var i = 0; i < this.keyQueue.length; i++){ 
			if ( this.keyQueue[i] === "Down") {
			  this.keyQueue.splice(i, 1); 
			}
		 }
	}
	if (cursors.left.isDown){
		if (!this.keyQueue.includes("Left")){
			this.keyQueue.unshift("Left");
		}
	}
	else {
		for( var i = 0; i < this.keyQueue.length; i++){ 
			if ( this.keyQueue[i] === "Left") {
			  this.keyQueue.splice(i, 1); 
			}
		 }
	}
	if (cursors.right.isDown){
		if (!this.keyQueue.includes("Right")){
			this.keyQueue.unshift("Right");
		}
	}
	else {
		for( var i = 0; i < this.keyQueue.length; i++){ 
			if ( this.keyQueue[i] === "Right") {
			  this.keyQueue.splice(i, 1); 
			}
		 }
	}
	if(this.keyQueue.length>0 && this.keyQueue[0]==="Up"){
			if(!this.move){
	...
	if(this.keyQueue.length>0 && this.keyQueue[0]==="Left"){
		...
		if(this.keyQueue.length>0 && this.keyQueue[0]==="Right"){
		...
		if(this.keyQueue.length>0 && this.keyQueue[0]==="Down"){
...
Players.prototype.hitWall = function(bullet, wallLayer){
	bullet.kill();
}
